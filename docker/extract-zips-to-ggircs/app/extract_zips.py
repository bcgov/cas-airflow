...
zip_passwords = list(map(passwd_to_byte, json.loads(os.getenv('ECCC_ZIP_PASSWORDS'))))

# tries to open the fil without password and then iterates through the list of passwords
def read_xml_file(fin, path):
  for passwd in [None] + zip_passwords:
    try:
...
for file in storage_client.list_blobs(gcs_bucket_name):
  ...
  try:
    ...
    with open(f"gs://{gcs_bucket_name}/{file.name}", 'rb', transport_params=dict(client=storage_client)) as fin:
